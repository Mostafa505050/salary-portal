// _worker.js - يدعم جميع الأسماء: TURSO_URL, TURSO_URLL, TURSO_URLI, TURSO_TOKEN, TOKENL, TOKENI
// لا تحتاج لمسح أي شيء - فقط أضف TURSO_URLL وسيعمل!
...
// يقرأ من أي اسم موجود - يدعم جميع الاختلافات
...
async function getBlockedFromTurso(env) {
  // يدعم: TURSO_URL, TURSO_URLL, TURSO_URLI, TURSO_URL_L, LIBSQL_URL, TURSO_URI
  const urlNames = ['TURSO_URL', 'TURSO_URLL', 'TURSO_URLI', 'TURSO_URL_L', 'LIBSQL_URL', 'TURSO_URI', 'LIBSQL_URLL'];
  const tokenNames = ['TURSO_TOKEN', 'TURSO_TOKENL', 'TURSO_TOKENI', 'TURSO_TOKEN_L', 'LIBSQL_TOKEN', 'TURSO_AUTH_TOKEN', 'LIBSQL_TOKENL'];
  
  let url = getEnvValue(env, urlNames);
  let token = getEnvValue(env, tokenNames);
  
  if (!url || !token || url.length < 10 || token.length < 20) return null;
    ...
    if (res.ok) {
      ...
      if (data && data[0] && data[0].results) {
        ...
        if (idx>=0) {
          ...
          return {pages, urlUsed: getEnvValue(env, urlNames) ? urlNames.find(n=>env[n]) : 'none', tokenUsed: tokenNames.find(n=>env[n]) || 'none'};
        }
      }
    }
...
async function getBlockedPages(env) {
  ...
  const result = await getBlockedFromTurso(env);
  if (result && result.pages && result.pages.length>0) {
    cache={pages:result.pages, time:Date.now()};
    return result.pages;
  }
  if (result && result.pages) {
    // حتى لو فارغة، يعني الاتصال نجح - استخدمها (قد تكون كل الصفحات مفعلة=1)
    cache={pages:result.pages, time:Date.now()};
    return result.pages;
  }
...
function fantasyBlockedPage(pageName, blockedList, envInfo) {
...
</style></head><body><div class="stars"></div><div class="box"><div class="ph"><div class="phw"><div class="phg"></div><div class="phb"></div><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"><div class="phd">🔒</div></div></div><div class="ct"><h1>الصفحة مغلقة</h1><div class="pn">`+pageName+`</div><div class="env `+envInfo.hasEnv+`">`+envInfo.label+`</div><p class="msg">عذراً، هذه الصفحة مغلقة حالياً من قبل الإدارة.<br>تم تعطيلها من لوحة التحكم (مفعلة=0) في جدول صفحات_الموقع.</p><div class="src">Blocked: `+blockedList.join(', ')+`<br>Vars: `+envInfo.foundVars+`</div><div class="fts"><div class="ft">🛡️ حماية من السيرفر</div><div class="ft">✨ تصميم خيالي</div><div class="ft">🔒 مفعلة=0</div><div class="ft">`+envInfo.usedName+`</div></div><a href="/" class="hb">🏠 العودة للرئيسية</a></div></div></body></html>`;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    function getFoundVars() {
      const found = [];
      for (const n of [...urlNames, ...tokenNames]) if (env[n]) found.push(n);
    ...
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        ...
        const hasUrl = urlNames.some(n=>!!env[n]);
        const hasToken = tokenNames.some(n=>!!env[n]);
        const hasEnv = hasUrl && hasToken;
        const usedUrl = urlNames.find(n=>!!env[n]) || 'none';
        const usedToken = tokenNames.find(n=>!!env[n]) || 'none';
        ...
        return new Response(JSON.stringify({
          ...
          usedUrl,
          usedToken,
          foundVars: getFoundVars(),
          supports: [...urlNames, ...tokenNames],
          source: hasEnv ? 'turso-direct-multi-names' : 'fallback',
          note: hasEnv ? '✅ يدعم TURSO_URL و TURSO_URLL و TURSO_URLI' : '⚠️ أضف TURSO_URLL في Workers Settings - يدعم جميع الأسماء',
    ...
    if (isHtml) {
      ...
      const hasToken = tokenNames.some(n=>!!env[n]);
      const hasEnv = hasUrl && hasToken;
      const usedUrl = urlNames.find(n=>!!env[n]) || 'fallback';
      
      if (isBlocked(pageName, blockedList)) {
        const envInfo = {
          hasEnv: String(hasEnv),
          label: hasEnv ? '✅ ' + usedUrl + ' - يدعم TURSO_URLL' : '⚠️ احتياطي',
          foundVars: getFoundVars(),
          usedName: usedUrl
        };
        return new Response(fantasyBlockedPage(pageName, blockedList, envInfo), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'multi-names-'+usedUrl}
        });
      }
    }
