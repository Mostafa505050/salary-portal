// _worker.js - V8 FINAL FIXED - index.html ONLY - NO REDIRECT LOOP - BUILD OK
...
const DEFAULT_PEPPER = "fallback_pepper_v8_please_set_PEPPER_SECRET";
...
function addSecurityHeaders(r){ try{ const h=new Headers(r.headers); for(const [k,v] of Object.entries(SECURITY_HEADERS)) if(!h.has(k)) h.set(k,v); return new Response(r.body,{status:r.status,headers:h}); }catch{ return r; } }
function getPepperSecret(env){ const p=(env.PEPPER_SECRET||env.PEPPER||'').trim(); return p.length>=8? p : DEFAULT_PEPPER; }
async function hashWithPepper(ch,nid,pep){ try{ const d=new TextEncoder().encode(String(ch).trim()+'|'+String(nid).trim()+'|'+pep); const b=await crypto.subtle.digest('SHA-256',d); return Array.from(new Uint8Array(b)).map(x=>x.toString(16).padStart(2,'0')).join(''); }catch{ return String(ch); } }
async function hashPatternClient(v,nid){ try{ const c=String(v).replace(/[^0-9,]/g,'').substring(0,50); const d=new TextEncoder().encode(c+'|'+String(nid).trim()); const b=await crypto.subtle.digest('SHA-256',d); return Array.from(new Uint8Array(b)).map(x=>x.toString(16).padStart(2,'0')).join(''); }catch{ return String(v); } }
async function hashPatternServer(ch,nid,pep){ return await hashWithPepper(ch,nid,pep); }
async function hashPasswordSecure(pw,nid,pep){ try{ const d=new TextEncoder().encode(String(pw).trim()+'|'+String(nid).trim()+'|'+pep); const b=await crypto.subtle.digest('SHA-256',d); return Array.from(new Uint8Array(b)).map(x=>x.toString(16).padStart(2,'0')).join(''); }catch{ return String(pw); } }
...
function hasValidEntry(req){ const ck=getCookieFixed(req,'entry_via_index'); if(ck==='1') return true; try{ const r=req.headers.get('Referer')||''; if(r.toLowerCase().includes('index.html')||r.endsWith('/')||r.includes('/index')) return true; }catch{} return false; }
function isEntryHtmlPage(path){ const low=path.toLowerCase(); const blocked=['/api/','/js/','.js','.css','.json','.png','.jpg','.svg','.ico','get-ip','turso','favicon','debug-config','security-stats']; for(const s of blocked) if(low.includes(s)) return false; let pn=path.split('/').pop()||''; if(path==='/'||path===''||pn==='') return false; if(pn.toLowerCase()==='index.html') return false; if(path.endsWith('.html')) return true; if(!pn.includes('.')&&!path.startsWith('/api/')) return true; return false; }
function entryBlockedHTMLFixed(p){ return `<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>الدخول عبر الرئيسية</title><style>body{min-height:100vh;background:#0a0e1a;color:#fff;display:flex;align-items:center;justify-content:center;font-family:Cairo}.box{background:rgba(255,255,255,0.08);padding:40px;border-radius:24px;text-align:center}h1{color:#f59e0b}</style></head><body><div class="box"><h1>الدخول عبر الرئيسية فقط</h1><p>${p}</p><a href="/index.html" style="color:#10b981">الرئيسية</a></div></body></html>`; }
function addEntryCookieToResponse(r){ try{ const h=new Headers(r.headers); h.set('Set-Cookie','entry_via_index=1; Path=/; Max-Age=3600; SameSite=Lax'); h.set('Cache-Control','no-cache'); for(const [k,v] of Object.entries(SECURITY_HEADERS)) h.set(k,v); return new Response(r.body,{status:r.status,headers:h}); }catch{ return r; } }
...
function generateSecureTokenFixed(){ try{ const a=new Uint8Array(SECURE_LOGIN_CONFIG.TOKEN_BYTES); crypto.getRandomValues(a); let hex=''; for(let i=0;i<a.length;i++) hex+=a[i].toString(16).padStart(2,'0'); return `sec_${hex}_${Date.now()}_${crypto.randomUUID()}`; }catch{ return `sec_${Math.random().toString(36).slice(2)}_${Date.now()}`; } }
function safeRowToObject(cols,row){ const o={}; row.forEach((cell,i)=>{ const k=cols[i]; if(!k) return; if(k.includes('__proto__')||k.includes('constructor')) return; o[k]=cell.value??cell.text??''; }); return o; }
...
function getTursoConfig(env){ let url=(env.TURSO_URL||HARDCODED_TURSO_URL||'').trim(); let token=(env.TURSO_TOKEN||HARDCODED_TURSO_TOKEN||'').trim(); if(!url) return {url:null,token:null,error:'TURSO_URL غير موجود'}; if(!token||token.length<10) return {url:null,token:null,error:'TURSO_TOKEN فارغ - ضعه في Settings > Variables'}; if(url.startsWith('libsql://')) url='https://'+url.slice(8); if(url&&!url.startsWith('https://')) url='https://'+url; return {url,token,error:null}; }
async function tursoQuery(env,sql,params=[]){ const cfg=getTursoConfig(env); if(cfg.error) return {error:cfg.error,isConfigError:true}; try{ const args=params.map(v=>({type:'text',value:String(v)})); const res=await fetch(`${cfg.url}/v2/pipeline`,{method:'POST',headers:{'Authorization':`Bearer ${cfg.token}`,'Content-Type':'application/json'},body:JSON.stringify({requests:[{type:'execute',stmt:{sql,args}},{type:'close'}]})}); if(!res.ok){ const t=await res.text().catch(()=>res.statusText); return {error:`Turso HTTP ${res.status}: ${t.substring(0,200)}`}; } const data=await res.json(); if(data.results?.[0]?.error) return {error:data.results[0].error.message}; return {result:data.results?.[0]?.response?.result}; }catch(e){ return {error:`Turso: ${e.message}`}; } }
function getClientIp(req){ return req.headers.get('CF-Connecting-IP')||req.headers.get('X-Forwarded-For')?.split(',')[0]?.trim()||'unknown'; }
function checkIpBlockedMemory(ip){ if(ip==='unknown') return {blocked:false}; const e=IP_BLOCK_MAP.get(ip); if(!e) return {blocked:false}; const now=Date.now(); if(e.blockedUntil&&now<e.blockedUntil) return {blocked:true,secondsLeft:Math.ceil((e.blockedUntil-now)/1000)}; if(e.blockedUntil&&now>=e.blockedUntil){ IP_BLOCK_MAP.delete(ip); return {blocked:false}; } return {blocked:false}; }
function recordFailedAttemptMemory(ip){ const now=Date.now(); const e=IP_BLOCK_MAP.get(ip)||{attempts:0,blockedUntil:0}; e.attempts+=1; e.lastAttempt=now; if(e.attempts>=5) e.blockedUntil=now+300000; IP_BLOCK_MAP.set(ip,e); return e.attempts; }
...
async function fetchAssetWithCleanUrls(request, env){
  ...
  if(!hasExt&&path!=='/'){ candidates.push(path+'.html'); candidates.push(path+'/index.html'); }
  ...
  for(const candPath of candidates){
    try{
      ...
      if(env.ASSETS){ const res=await env.ASSETS.fetch(new Request(candUrl, request)); if(res.status!==404) return res; }
    }catch{ continue; }
  }
...
export default {
  async fetch(request, env, ctx){
    ...
    const sensitive=['/api/check-by-card-secure','/api/verify-password-secure','/api/bio-login-by-card-secure','/api/register-by-card-secure'];
    if(sensitive.includes(path)){ const bc=checkIpBlockedMemory(clientIp); if(bc.blocked) return new Response(JSON.stringify({ok:false,msg:`🔒 محظور ${bc.secondsLeft} ثانية`,blocked:true}),{status:429,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }

    if(path==='/api/debug-config'){
      const cfg=getTursoConfig(env); const pepper=getPepperSecret(env);
      return new Response(JSON.stringify({v:'V8-FINAL-FIXED-NO-LOOP-index.html',tursoUrl:cfg.url?cfg.url.substring(0,40)+'...':'missing',hasToken:!!cfg.token,configError:cfg.error,hasPepper:pepper!==DEFAULT_PEPPER,hasAssets:!!env.ASSETS,ip:clientIp,time:new Date().toISOString()},null,2),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    }
    if(path==='/api/get-ip'){ return new Response(JSON.stringify({ip:clientIp}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}}); }
    ...
    if(path==='/api/check-by-card-secure'&&request.method==='POST'){
      try{
        ...
        if(!isValidNationalIdSecure(card)) return new Response(JSON.stringify({ok:false,msg:'رقم البطاقة 14 رقم'}),{status:400,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
        let q=await tursoQuery(env,`SELECT * FROM "موظفين_مرتبات" WHERE "الرقم_القومى"=? LIMIT 1`,[card]);
        ...
        if(q.error){ let q2=await tursoQuery(env,`SELECT * FROM "موظفين_مرتبات" WHERE "الرقم_القومي"=? LIMIT 1`,[card]); if(q2.error) return new Response(JSON.stringify({ok:false,msg:q.error}),{status:500,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); q=q2; }
        ...
        const cols=q.result.cols.map(c=>c.name); const u=safeRowToObject(cols,q.result.rows[0]); delete u['كلمة_المرور']; return new Response(JSON.stringify({ok:true,user:u}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    ...
    if(path==='/api/verify-password-secure'&&request.method==='POST'){
      try{
        const body=await request.json().catch(()=>({})); const card=String(body.cardNumber||'').trim(); const pw=String(body.password||'').trim();
        ...
        const cols=q.result.cols.map(c=>c.name); const u=safeRowToObject(cols,q.result.rows[0]); const stored=String(u['كلمة_المرور']||'').trim(); const pepper=getPepperSecret(env); const attempt=await hashPasswordSecure(pw,card,pepper); let ok=false; if(stored.length===64) ok=constantTimeCompare(stored,attempt); else ok=constantTimeCompare(stored,pw);
    ...
    if(path==='/api/bio-login-by-card-secure'&&request.method==='POST'){
      try{
        ...
        const cols=q.result.cols.map(c=>c.name); const u=safeRowToObject(cols,q.result.rows[0]); const bioStr=String(u['بصمة']||'').trim(); if(!bioStr) return new Response(JSON.stringify({ok:false,msg:'لا توجد بصمة'}),{status:400,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
        let saved; try{ saved=JSON.parse(bioStr); }catch{ return new Response(JSON.stringify({ok:false,msg:'بيانات تالفة'}),{status:500,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }
        const pepper=getPepperSecret(env); let ok=false; if(saved.serverHash&&bioAttempt){ const ah=await hashPatternServer(bioAttempt,card,pepper); if(constantTimeCompare(ah,saved.serverHash)) ok=true; } if(!ok&&bioAttempt&&saved.hash&&constantTimeCompare(saved.hash,bioAttempt)) ok=true;
    ...
    if(path==='/api/register-by-card-secure'&&request.method==='POST'){
      try{
        const body=await request.json().catch(()=>({})); const card=String(body.cardNumber||'').trim(); let bio=String(body.bio||'').trim(); const pw=String(body.password||'').trim(); const pepper=getPepperSecret(env); const hashedPw=await hashPasswordSecure(pw,card,pepper);
        try{ const bj=JSON.parse(bio); if(bj.hash){ const sh=await hashPatternServer(bj.hash,card,pepper); bj.serverHash=sh; bj.hash=undefined; bio=JSON.stringify(bj); } }catch{}
        const up=await tursoQuery(env,`UPDATE "موظفين_مرتبات" SET "بصمة"=?, "كلمة_المرور"=? WHERE "الرقم_القومى"=?`,[bio,hashedPw,card]); if(up.error) return new Response(JSON.stringify({ok:false,msg:up.error}),{status:500,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    ...
    let response;
    try{
      const ar=await fetchAssetWithCleanUrls(request,env);
      if(ar&&ar.status!==404) response=ar;
    ...
    if(!response||response.status===404){
      ...
      return new Response(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>404</title></head><body style="background:#020a05;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh"><div style="text-align:center"><h1>404</h1><p>${path}</p><a href="/index.html" style="color:#10b981">الرئيسية index.html</a></div></body></html>`,{status:404,headers:{'Content-Type':'text/html; charset=utf-8',...SECURITY_HEADERS}});
    }
