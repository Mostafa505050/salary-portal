// _worker.js FINAL v17 - يقرأ مباشرة من Turso libSQL بدون وسيط
// يحتاج TURSO_URL و TURSO_TOKEN في Cloudflare Pages Env Vars

const PHOTO = "data:image/jpeg;base64,";

let cache = {pages:[], time:0};
const TTL = 15 * 1000;

async function queryTursoDirect(env, sql) {
  // جرب أولاً env vars
  const url = env.TURSO_URL || env.LIBSQL_URL || "";
  const token = env.TURSO_TOKEN || env.LIBSQL_TOKEN || env.TURSO_AUTH_TOKEN || "";
  
  if (url && token) {
    try {
      const httpUrl = url.replace('libsql://', 'https://').replace('wss://','https://');
      const res = await fetch(httpUrl, {
        method:'POST',
        headers:{
          'Authorization': 'Bearer '+token,
          'Content-Type':'application/json'
        },
        body: JSON.stringify({statements:[sql]})
      });
      if (res.ok) {
        const data = await res.json();
        // libSQL HTTP API format
        if (data && data[0] && data[0].results) {
          const result = data[0].results;
          const cols = result.columns || [];
          const rows = result.rows || [];
          return rows.map(r=>{
            let o={};
            r.forEach((v,i)=>{ o[cols[i]]=v; });
            return o;
          });
        }
        if (data && data.results) {
          return data.results;
        }
        return data;
      }
    } catch(e) {
      console.log('[v17] Direct Turso failed:', e.message);
    }
  }
  return null;
}

async function queryViaAPI(sql) {
  const apis = [
    "https://auth-api.mostafa-voic77729.workers.dev/api/turso",
    "https://turso-api.mostafa-voic77729.workers.dev/api/turso"
  ];
  for (const api of apis) {
    try {
      const res = await fetch(api, {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({sql})
      });
      if (!res.ok) continue;
      const txt = await res.text();
      let data; try { data = JSON.parse(txt); } catch { continue; }
      let rows = data.rows || [];
      if (data.results && data.results[0]?.response?.result) {
        const cols = data.results[0].response.result.cols.map(c=>c.name);
        rows = data.results[0].response.result.rows.map(r=>{ let o={}; r.forEach((c,i)=>{o[cols[i]]=c.value ?? c.text ?? "";}); return o; });
      }
      if (rows.length>0) return rows;
    } catch(e) { continue; }
  }
  return [];
}

async function getBlockedPages(env) {
  if (Date.now() - cache.time < TTL && cache.pages.length>0) return cache.pages;

  // 1. جرب Turso مباشر
  const directRows = await queryTursoDirect(env, `SELECT "اسم_الصفحة", "مفعلة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`);
  if (directRows && directRows.length>0) {
    const pages = directRows.map(r=>String(r["اسم_الصفحة"]||r["اسم_الصفحه"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) {
      cache = {pages, time:Date.now()};
      return pages;
    }
  }

  // 2. جرب via API
  const rows = await queryViaAPI(`SELECT "اسم_الصفحة", "مفعلة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`);
  if (rows.length>0) {
    const pages = rows.map(r=>String(r["اسم_الصفحة"]||r["اسم_الصفحه"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) {
      cache = {pages, time:Date.now()};
      return pages;
    }
  }

  // 3. جرب كل الصفحات وفلتر
  const allRows = await queryViaAPI(`SELECT * FROM "صفحات_الموقع" LIMIT 50`);
  if (allRows.length>0) {
    const blocked = allRows.filter(r=>{ const m=r["مفعلة"]??r["مفعله"]; return m==0||m=="0"; });
    const pages = blocked.map(r=>String(r["اسم_الصفحة"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) {
      cache = {pages, time:Date.now()};
      return pages;
    }
  }

  return cache.pages;
}

async function isBlocked(env, pageName) {
  const low = pageName.toLowerCase().replace('.html','').trim();
  const pages = await getBlockedPages(env);
  for (const b of pages) {
    const bl = b.toLowerCase().replace('.html','').trim();
    if (bl===low || bl===low+'.html' || low.includes(bl) || bl.includes(low)) {
      return {blocked:true, source:'turso-direct:'+b, blockedList:pages};
    }
  }
  return {blocked:false, source:'not-in-list', blockedList:pages};
}

function blockedHTML(pName, info) {
  return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>مغلقة</title><style>
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:#0a0a1a;background:radial-gradient(ellipse at top,#1a1a40 0%,#0a0a1a 50%,#000 100%);font-family:Tajawal,sans-serif;direction:rtl;display:flex;align-items:center;justify-content:center}
.box{display:flex;gap:32px;align-items:center;background:rgba(255,255,255,0.07);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.12);border-radius:24px;padding:32px;max-width:800px;width:92%}.phw{width:180px;height:180px}.phi{width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid white}.pn{font-size:14px;color:#a78bfa;background:rgba(167,139,250,0.15);padding:6px 12px;border-radius:8px;display:inline-block;margin-bottom:10px;font-family:monospace;direction:ltr}.src{font-size:10px;color:#9ca3af;background:rgba(0,0,0,0.3);padding:6px;border-radius:6px;margin-bottom:10px;word-break:break-all}
</style></head><body><div class="box"><div class="phw"><img src="`+PHOTO+`" class="phi"></div><div class="ct"><h2>الصفحة مغلقة</h2><div class="pn">`+pName+`</div><p>تم تعطيلها من لوحة التحكم (مفعلة=0)</p><div class="src">`+(info.source||'')+`<br>`+(info.blockedList?info.blockedList.join(', '):'')+`</div><a href="/" style="color:white;background:#7850ff;padding:10px 20px;border-radius:10px;text-decoration:none;display:inline-block;margin-top:10px">🏠 الرئيسية</a></div></div></body></html>`;
}

export default {
  async fetch(req, env, ctx) {
    const url = new URL(req.url);
    const path = url.pathname;

    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        const pages = await getBlockedPages(env);
        const hasEnv = !!(env.TURSO_URL || env.LIBSQL_URL);
        return new Response(JSON.stringify({blocked:pages, count:pages.length, hasTursoEnv:hasEnv, envUrl: env.TURSO_URL ? env.TURSO_URL.slice(0,30)+'...' : 'missing', time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
      if (path === '/api/debug') {
        const q = url.searchParams.get('page') || 'tables.html';
        const pages = await getBlockedPages(env);
        const info = await isBlocked(env, q);
        return new Response(JSON.stringify({query:q, blockedList:pages, isBlocked:info, hasTursoEnv:!!(env.TURSO_URL||env.LIBSQL_URL), time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
    }

    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar'];
    if (skip.some(s=>path.toLowerCase().includes(s.toLowerCase()))) {
      return await env.ASSETS.fetch(req);
    }

    let pageName = path.split('/').pop() || 'index.html';
    if (path === '/' || path === '') pageName = 'index.html';
    const isHtml = path.endsWith('.html') || path==='/' || path==='' || (!path.includes('.') && !path.startsWith('/api/'));

    if (isHtml) {
      const info = await isBlocked(env, pageName);
      if (info.blocked) {
        return new Response(blockedHTML(pageName, info), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'v17-direct-turso'}
        });
      }
    }

    return env.ASSETS.fetch(req);
  }
}
