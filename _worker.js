// _worker.js - طريقة مختلفة - TURSO_URL و TOKEN مضمنة في الكود نفسه
// لا تحتاج إضافة في Workers Settings - تعمل مباشرة
...
// ضع بياناتك هنا مباشرة - طريقة مختلفة
const HARDCODED_TURSO_URL = "libsql://company-alldata-mostafadarwish-mostafa505050.aws-eu-west-1.turso.io";
const HARDCODED_TURSO_TOKEN = "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3ODYzNTgyODIsImlkIjoiMDE5ZmU2NjEtYzkwMS03ZjMyLWE2NDQtNWIwMTQyMzc3MjdkIiwia2lkIjoiN2JieU4xMTB1VEhfNXRJaTZPR3ZQc2oxOEJ5YWU2ZnF1eUdrMXJMYmZwVSIsInJpZCI6IjdjODI4MWExLWRlZTktNGE5Ny04OGJmLTU5ZTY2OWE5NzA5NiJ9.Gt2LMd2m70RGIal4nUpZGAdp0cwBBSBWqfuhpSu8Asp-H5RjM-I157zkNOqtTq0dwcNE_lO_9iUxehNZ3LqqCA"; // ضع التوكن الكامل هنا

const FALLBACK_BLOCKED = ['addhafez1.html', 'addhafez1', 'tables.html', 'tables', 'salaryold.html', 'salaryold'];

let cache = {pages:FALLBACK_BLOCKED, time:0};
const TTL = 15 * 1000;

async function getBlockedFromTurso(env) {
  // يقرأ أولاً من القيم المضمنة في الكود (الطريقة المختلفة)
  let url = HARDCODED_TURSO_URL || env.TURSO_URL || env.LIBSQL_URL || "";
  let token = HARDCODED_TURSO_TOKEN || env.TURSO_TOKEN || env.LIBSQL_TOKEN || "";

  // إذا كانت القيمة تحتوي على ... فهذا يعني لم تضع التوكن - استخدم env
  if (url.includes("...") || token.includes("...") || token.length < 20) {
    url = env.TURSO_URL || env.LIBSQL_URL || url;
    token = env.TURSO_TOKEN || env.LIBSQL_TOKEN || token;
  }
  ...
  } catch(e) { console.log('Turso failed:', e.message); }
  return null;
}
...
async function getBlockedPages(env) {
  ...
  const direct = await getBlockedFromTurso(env);
  if (direct && direct.length>0) {
    cache={pages:direct, time:Date.now()};
    return direct;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (d.blocked && d.blocked.length>0) {
        const pages = d.blocked.map(s=>s.toLowerCase());
...
function fantasyBlockedPage(pageName, blockedList, hasEnv) {
...
</style></head><body><div class="stars"></div><div class="box"><div class="ph"><div class="phw"><div class="phg"></div><div class="phb"></div><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"><div class="phd">🔒</div></div></div><div class="ct"><h1>الصفحة مغلقة</h1><div class="pn">`+pageName+`</div><div class="env `+hasEnv+`">hasEnv: `+hasEnv+` `+(hasEnv==='true'?'✅ ديناميكي من صفحات_الموقع - طريقة مختلفة':'⚠️ احتياطي')+`</div><p class="msg">عذراً، هذه الصفحة مغلقة حالياً من قبل الإدارة.<br>تم تعطيلها من لوحة التحكم (مفعلة=0) في جدول صفحات_الموقع.</p><div class="src">Blocked: `+blockedList.join(', ')+`</div><div class="fts"><div class="ft">🛡️ حماية من السيرفر</div><div class="ft">✨ تصميم خيالي</div><div class="ft">🔒 مفعلة=0</div></div><a href="/" class="hb">🏠 العودة للرئيسية</a></div></div></body></html>`;
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        ...
        const hasHardcoded = HARDCODED_TURSO_URL.includes('libsql://') && HARDCODED_TURSO_TOKEN.length > 50;
        const hasEnv = hasHardcoded || !!(env.TURSO_URL || env.LIBSQL_URL);
        return new Response(JSON.stringify({
          ...
          hasEnv,
          hasHardcoded,
          hasUrl: hasHardcoded || !!(env.TURSO_URL||env.LIBSQL_URL),
          hasToken: hasHardcoded || !!(env.TURSO_TOKEN||env.LIBSQL_TOKEN),
          source: hasHardcoded ? 'hardcoded-in-code-طريقة-مختلفة' : (hasEnv ? 'turso-direct' : 'fallback'),
          method: 'different-way-hardcoded',
          time:new Date().toISOString(),
          note: hasEnv ? '✅ ديناميكي من Turso (مضمن في الكود)' : '⚠️ احتياطي'
        }, null, 2), {
      ...
      if (path === '/api/debug') {
        ...
        const pages = await getBlockedPages(env);
        const blocked = isBlocked(q, pages);
        return new Response(JSON.stringify({
          query:q, 
          blocked,
          blockedList:pages,
          hasEnv:!!(HARDCODED_TURSO_URL || env.TURSO_URL),
          method:'hardcoded',
          time:new Date().toISOString()
        }, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
    ...
    if (isHtml) {
      const blockedList = await getBlockedPages(env);
      const hasHardcoded = HARDCODED_TURSO_URL.includes('libsql://') && HARDCODED_TURSO_TOKEN.length > 50;
      const hasEnv = String(hasHardcoded || !!(env.TURSO_URL || env.LIBSQL_URL));
      if (isBlocked(pageName, blockedList)) {
        return new Response(fantasyBlockedPage(pageName, blockedList, hasEnv), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'hardcoded-method'}
        });
      }
    }
