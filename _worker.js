// _worker.js FINAL v20 - حل كامل خطوة بخطوة - ديناميكي من Turso + خيالي + صورتك + بدون زر تحكم
// يقرأ من جدول صفحات_الموقع حيث مفعلة=0 منع، مفعلة=1 سماح
...
// قائمة احتياطية من صورتك image_c3958f.png: 2 صفحات معطلة
const FALLBACK_BLOCKED = ['AddHafez1.html', 'AddHafez1'];

let cache = {pages:[], time:0};
const TTL = 15 * 1000;

async function queryTurso(env, sql) {
  // 1. جرب قراءة مباشرة من Turso عبر env vars (كما في Database-Manager)
  const url = env.TURSO_URL || env.LIBSQL_URL || env.VITE_TURSO_URL || "";
  const token = env.TURSO_TOKEN || env.LIBSQL_TOKEN || env.VITE_TURSO_TOKEN || env.TURSO_AUTH_TOKEN || "";
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && data[0] && data[0].results) {
          ...
          return rows.map(r=>{ let o={}; r.forEach((v,i)=>{o[cols[i]]=v;}); return o; });
        }
      }
    } catch(e) { console.log('Direct Turso failed:', e.message); }
  }
  return null;
}

async function queryViaAPI(sql) {
  const apis = [
    "https://auth-api.mostafa-voic77729.workers.dev/api/turso",
...
async function getBlockedPages(env) {
  if (Date.now() - cache.time < TTL && cache.pages.length>0) return cache.pages;

  // 1. Turso مباشر عبر env vars
  const direct = await queryTurso(env, `SELECT "اسم_الصفحة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`);
  if (direct && direct.length>0) {
    const pages = direct.map(r=>String(r["اسم_الصفحة"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) { cache={pages, time:Date.now()}; return pages; }
  }

  // 2. عبر API
  const rows = await queryViaAPI(`SELECT "اسم_الصفحة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`);
  if (rows.length>0) {
    const pages = rows.map(r=>String(r["اسم_الصفحة"]||r["اسم_الصفحه"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) { cache={pages, time:Date.now()}; return pages; }
  }

  // 3. كل الصفحات وفلتر
  const all = await queryViaAPI(`SELECT * FROM "صفحات_الموقع" LIMIT 50`);
  if (all.length>0) {
    const blocked = all.filter(r=>{ const m=r["مفعلة"]??r["مفعله"]; return m==0||m=="0"; });
    const pages = blocked.map(r=>String(r["اسم_الصفحة"]||"").toLowerCase().trim()).filter(Boolean);
    if (pages.length>0) { cache={pages, time:Date.now()}; return pages; }
  }

  // 4. احتياطي من صورتك
  cache={pages:FALLBACK_BLOCKED, time:Date.now()};
  return FALLBACK_BLOCKED;
}

async function isBlocked(env, pageName) {
  const low = pageName.toLowerCase().replace('.html','').trim();
  const pages = await getBlockedPages(env);
  for (const b of pages) {
    const bl = b.toLowerCase().replace('.html','').trim();
    if (bl===low || bl===low+'.html' || low.includes(bl) || bl.includes(low)) {
      return {blocked:true, source:'turso:'+b, blockedList:pages};
    }
  }
  return {blocked:false, source:'allowed', blockedList:pages};
}

function blockedPageHTML(pName, info) {
...
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:#0a0a1a;background:radial-gradient(ellipse at top,#1a1a40 0%,#0a0a1a 50%,#000 100%);font-family:Tajawal,Cairo,sans-serif;direction:rtl;display:flex;align-items:center;justify-content:center;overflow:hidden}
...
</style></head><body><div class="stars"></div><div class="box"><div class="ph"><div class="phw"><div class="phg"></div><div class="phb"></div><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"><div class="phd">🔒</div></div></div><div class="ct"><h1>الصفحة مغلقة</h1><div class="pn">`+pName+`</div><p class="msg">عذراً، هذه الصفحة مغلقة حالياً من قبل الإدارة.<br>تم تعطيلها من لوحة التحكم (مفعلة=0).</p><div class="src">Source: `+info.source+`<br>All blocked: `+info.blockedList.join(', ')+`</div><div class="fts"><div class="ft">🛡️ حماية إجبارية</div><div class="ft">✨ تصميم خيالي</div><div class="ft">🔒 Turso ديناميكي</div></div><a href="/" class="hb">🏠 العودة للرئيسية</a></div></div></body></html>`;
}
  ...
  async fetch(req, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        const pages = await getBlockedPages(env);
        return new Response(JSON.stringify({blocked:pages, count:pages.length, hasEnv:!!(env.TURSO_URL||env.LIBSQL_URL), time:new Date().toISOString()}, null, 2), {
      ...
      if (path === '/api/debug') {
        ...
        const info = await isBlocked(env, q);
        return new Response(JSON.stringify({query:q, isBlocked:info, time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
    }
    ...
    // صفحات لا يتم حجبها
    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar', 'protect.js'];
    ...
    if (isHtml) {
      const info = await isBlocked(env, pageName);
      if (info.blocked) {
        return new Response(blockedPageHTML(pageName, info), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache, no-store','X-Blocked-By':'v20-dynamic-fantasy'}
        });
      }
    }
