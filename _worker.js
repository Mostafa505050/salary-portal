// _worker.js - V9.2 ULTRA FULL MERGED - دمج كامل قديم+جديد + يبني 100%
...
const SECURITY_HEADERS = {
  ...
  'X-Powered-By':'SecurePayslip-V9.2-FULL','Cache-Control':'no-store, no-cache, must-revalidate',
};
...
function getPepperSecret(env){ const p=(env.PEPPER_SECRET||'').trim(); if(p.length>=16) return p; return "fallback_pepper_v9_set_PEPPER_SECRET"; }
...
async function tursoQuery(env,sql,params=[]){
  ...
  try{
    ...
    const txt=await res.text(); let data; try{ data=JSON.parse(txt); }catch{ return {error:`HTTP ${res.status}: ${txt.slice(0,200)}`}; }
...
export default {
  async fetch(request, env, ctx){
    ...
    if(path==='/api/debug-config'){
      const cfg=getTursoConfig(env); const countQ=await tursoQuery(env, `SELECT COUNT(*) as c FROM "موظفين_مرتبات"`); let count=countQ.error? countQ.error : '0'; if(countQ.result?.rows?.length){ const cols=countQ.result.cols.map(c=>c.name); count=safeRowToObject(cols,countQ.result.rows[0]).c; }
      return new Response(JSON.stringify({v:'V9.2-FULL-MERGED', tursoOk:!cfg.error, tableCount:count, ip:clientIp},null,2),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    }
    ...
    if(path==='/api/security-stats'){ const q1=await tursoQuery(env, `SELECT COUNT(*) as cnt FROM failed_logins WHERE blocked_until >?`, [String(Date.now())]); let c=0; if(q1.result?.rows?.length){ const cols=q1.result.cols.map(x=>x.name); c=Number(safeRowToObject(cols,q1.result.rows[0]).cnt||0); } return new Response(JSON.stringify({blockedCount:c}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }
    ...
    if(path==='/api/check-by-card-secure' && request.method==='POST'){
      ...
      return new Response(JSON.stringify({ok:true,user:u, sqlUsed:foundRes.sqlUsed}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    }
    ...
    if(path==='/api/verify-password-secure' && request.method==='POST'){
      ...
      let ok=constantTimeCompare(stored,hashed)||constantTimeCompare(stored,pass); if(!ok){ const att=await recordFailedAttempt(env,clientIp); return new Response(JSON.stringify({ok:false,msg:`بيانات غير صحيحة بقي ${5-att}`}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }
    ...
    if(path==='/api/register-by-card-secure' && request.method==='POST'){
      ...
      try{ const b=JSON.parse(bio); if(b.type==='pattern'&&b.hash){ const sh=await hashPatternServer(b.hash,card,pepper); b.serverHash=sh; b.hash=undefined; b.value=undefined; b.secure=true; b.v10=true; bio=JSON.stringify(b); } if(b.type==='face_camera'&&b.hash){ const sh=await hashFaceServer(b.hash,card,pepper); b.serverHash=sh; b.hash=undefined; b.secure=true; b.v10=true; bio=JSON.stringify(b); } }catch{}
    ...
    if(path==='/api/bio-login-by-card-secure' && request.method==='POST'){
      ...
      const foundRes=await findEmployeeByCard(env, card); if(!foundRes.found) return new Response(JSON.stringify({ok:false,msg:'غير موجود'}),{status:400,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
      const cols=foundRes.q.result.cols.map(c=>c.name); const u=safeRowToObject(cols,foundRes.q.result.rows[0]); const savedStr=String(u['بصمة']||'').trim(); let saved; try{ saved=JSON.parse(savedStr); }catch{ return new Response(JSON.stringify({ok:false,msg:'بيانات تالفة'}),{status:500,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }
      const pepper=getPepperSecret(env); let ok=false;
      if(saved.type==='pattern'){ const ah=await hashPatternServer(bioAttempt,card,pepper); ok=constantTimeCompare(ah, saved.serverHash||saved.hash||''); if(!ok&&saved.value) ok=constantTimeCompare(String(saved.value).trim(), bioAttempt.trim()); }
      else if(saved.type==='face_camera'){ const ah=await hashFaceServer(bioAttempt,card,pepper); ok=constantTimeCompare(ah, saved.serverHash||saved.hash||''); }
      if(!ok){ const att=await recordFailedAttempt(env,clientIp); return new Response(JSON.stringify({ok:false,msg:`غير مطابق بقي ${5-att}`}),{status:400,headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}}); }
      await clearFailedAttempts(env,clientIp); const token=generateSecureTokenFixed();
      return new Response(JSON.stringify({ok:true,token,role:u['الصلاحيات']||'User'}),{headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',...SECURITY_HEADERS}});
    }
