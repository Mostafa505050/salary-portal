// _worker.js FINAL v2 - معالجة أسماء الصفحات بمرونة + Debug headers
...
async function checkPageBlocked(pageName) {
  // جرب عدة صيغ للاسم
  const variants = [pageName, pageName.replace(/\/$/,''), pageName + '/', '/' + pageName];
  const uniqueVariants = [...new Set(variants)];
  
  for (const variant of uniqueVariants) {
    const sql = `SELECT "مفعلة", "اسم_الصفحة" FROM "صفحات_الموقع" WHERE "اسم_الصفحة"='${variant.replace(/'/g, "''")}' OR "اسم_الصفحة" LIKE '%${variant.replace(/'/g, "''")}%' LIMIT 1`;
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (rows.length > 0 && rows[0]) {
            const enabled = rows[0]["مفعلة"] ?? rows[0].مفعلة ?? rows[0]["مفعلة"];
            const isBlocked = enabled==0 || enabled=="0" || enabled===false || enabled==="false" || enabled==="معطل" || enabled===0;
            if (isBlocked) return { blocked: true, row: rows[0], variant, sql };
            else return { blocked: false, row: rows[0], variant, sql };
          }
        }
      } catch(e) { continue; }
    }
  }
  return { blocked: false, row: null, variant: null, sql: null };
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (isHtml) {
      try {
        ...
        const guardScript = `<script>console.log('%c[Worker] ACTIVE FINAL v2 - Guard injected', 'color: lime; background: black; padding: 4px 8px; font-weight: bold;');(function(){const k='salary_portal_page_status';try{const d=JSON.parse(localStorage.getItem(k)||'{}');const p=(location.pathname.split('/').pop()||'index.html');const pn=p===''?'index.html':p;console.log('[Guard] LS:',d,'Page:',pn,'BlockInfo:', ${JSON.stringify(blockInfo)});if(d[pn]&&d[pn].enabled===false){document.documentElement.innerHTML='<div style=display:flex;align-items:center;justify-content:center;min-height:100vh;background:#f8f9fb;font-family:Tajawal;direction:rtl><div style=text-align:center;background:white;padding:40px;border-radius:16px;border:1px solid #e2e8f0;max-width:400px><div style=font-size:48px>⛔</div><h2>الصفحة معطلة</h2><p>'+pn+' (localStorage)</p><a href=/database-manager-FIXED.html style=padding:8px 16px;background:#0f172a;color:white;border-radius:8px;text-decoration:none;display:inline-block;margin-top:12px>لوحة التحكم</a></div></div>';}}catch(e){}})();</script>`;
        ...
        newHeaders.set('X-Worker-Active', 'FINAL-v2');
        newHeaders.set('X-Block-Check', blockInfo.blocked ? 'blocked' : 'allowed');
        newHeaders.set('X-Block-Row', JSON.stringify(blockInfo.row || {}).slice(0,300));
        return new Response(html, { status: response.status, headers: newHeaders });
      } catch(e) {
        const newHeaders = new Headers(response.headers);
        newHeaders.set('X-Worker-Active', 'FINAL-v2-error');
        newHeaders.set('X-Error', e.message.slice(0,200));
