// _worker.js FINAL v14 - إصلاح نهائي 100%: Turso + خيالي + صورتك + بدون تحكم
// يصلح: /api/blocked-list كان يعتبر HTML ويتم حجبه
...
const CHECK_PRIMARY = "https://auth-api.mostafa-voic77729.workers.dev/api/check-page-status";

let blockedCache = {pages:[], time:0};
const TTL = 15 * 1000;

async function ft(url, opts, t=5000) {
...
async function getBlocked() {
  if (Date.now() - blockedCache.time < TTL && blockedCache.pages.length>0) return blockedCache.pages;
  
  // نفس الاستعلام الذي يعمل في Turso Dashboard كما في image_d0555b.png
  const sql = `SELECT "اسم_الصفحة", "مفعلة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`;
  ...
  for (const api of [TURSO_PRIMARY, TURSO_FALLBACK]) {
    try {
      ...
      const pages = rows.map(r=>String(r["اسم_الصفحة"]||r["اسم_الصفحه"]||r.name||"").toLowerCase().trim()).filter(Boolean);
      if (pages.length>=0) {
        blockedCache = {pages, time: Date.now()};
        console.log('[v14] Blocked:', pages);
        return pages;
      }
    } catch(e) { continue; }
  }
  return blockedCache.pages;
}

async function isBlocked(pageName) {
  const low = pageName.toLowerCase().replace('.html','').trim();
  
  // 1. API check
  try {
    const res = await ft(CHECK_PRIMARY, {
      method:'POST',
      headers:{'Content-Type':'application/json'},
    ...
    if (res.ok) {
      ...
      if (d.blocked === true) return {blocked:true, source:'check-api', row:d};
    }
  } catch(e) {}
  ...
  for (const b of list) {
    const bl = b.toLowerCase().replace('.html','').trim();
    if (bl === low || bl === (low+'.html') || low === bl || low.includes(bl) || bl.includes(low)) {
      return {blocked:true, source:'turso:'+b, row:{'اسم_الصفحة':b, 'مفعلة':0}, blockedList:list};
    }
  }
  return {blocked:false, source:'allowed', blockedList:list};
}
...
export default {
  async fetch(req, env, ctx) {
    ...
    // أهم شيء: /api/* لا تعتبر HTML أبداً
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        const list = await getBlocked();
        return new Response(JSON.stringify({blocked:list, count:list.length, time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache','X-Worker':'v14'}
        });
      }
      if (path === '/api/debug') {
        const q = url.searchParams.get('page') || 'tables.html';
        const list = await getBlocked();
        const info = await isBlocked(q);
        return new Response(JSON.stringify({query:q, blockedList:list, isBlocked:info, time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache','X-Worker':'v14'}
        });
      }
      if (path === '/api/check-page-status') {
        try {
          const body = await req.json();
          const info = await isBlocked(body.pageName || 'test');
          return new Response(JSON.stringify({blocked:info.blocked, source:info.source}), {
    ...
    // صفحات لوحة التحكم لا يتم حجبها
    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.jpeg', '.svg', '.ico', '.woff', '.woff2', 'fonts.googleapis', 'sidebar'];
    ...
    // فقط صفحات HTML يتم فحصها
    const isHtml = path.endsWith('.html') || path === '/' || path === '' || (!path.includes('.') && !path.startsWith('/api/'));

    if (isHtml) {
      const info = await isBlocked(pageName);
      if (info.blocked) {
        return new Response(pageHTML(pageName, info), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache, no-store','X-Blocked-By':'v14-Fantasy','X-Block-Source':info.source}
        });
      }
    }
