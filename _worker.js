// _worker.js FINAL v16 - إصلاح جذري: turso-api لم يعد فيه /api/turso
// الآن يعتمد فقط على auth-api/check-page-status الذي لا يزال يعمل

const CHECK_API = "https://auth-api.mostafa-voic77729.workers.dev/api/check-page-status";
const CHECK_API_FALLBACK = "https://turso-api.mostafa-voic77729.workers.dev/api/check-page-status";
const TURSO_DIRECT_PRIMARY = "https://auth-api.mostafa-voic77729.workers.dev/api/turso";
const TURSO_DIRECT_FALLBACK = "https://turso-api.mostafa-voic77729.workers.dev/api/turso";

let cache = {map:new Map(), time:0};
const TTL = 20 * 1000;

async function ft(url, opts, t=5000) {
  const c = new AbortController();
  const id = setTimeout(()=>c.abort(), t);
  try {
    const r = await fetch(url, {...opts, signal:c.signal});
    clearTimeout(id);
    return r;
  } catch(e) { clearTimeout(id); throw e; }
}

async function checkOnePage(pageName) {
  const key = pageName.toLowerCase();
  if (cache.map.has(key) && Date.now() - cache.time < TTL) {
    return cache.map.get(key);
  }

  // 1. جرب check-page-status API (الأساسي)
  for (const api of [CHECK_API, CHECK_API_FALLBACK]) {
    try {
      const res = await ft(api, {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({pageName: pageName})
      }, 4000);
      if (res.ok) {
        const txt = await res.text();
        try {
          const d = JSON.parse(txt);
          if (typeof d.blocked === 'boolean') {
            const result = {blocked:d.blocked, source:'check-api:'+api, row:d, api};
            cache.map.set(key, result);
            cache.time = Date.now();
            if (d.blocked) return result;
          }
        } catch(e) {}
      }
    } catch(e) {}
  }

  // 2. جرب Turso مباشر لنفس الصفحة
  const safe = pageName.replace(/'/g, "''");
  const sqls = [
    `SELECT "مفعلة", "اسم_الصفحة" FROM "صفحات_الموقع" WHERE "اسم_الصفحة"='${safe}' LIMIT 1`,
    `SELECT * FROM "صفحات_الموقع" WHERE "اسم_الصفحة" LIKE '%${safe.replace('.html','')}%' LIMIT 1`
  ];
  
  for (const sql of sqls) {
    for (const api of [TURSO_DIRECT_PRIMARY, TURSO_DIRECT_FALLBACK]) {
      try {
        const res = await ft(api, {
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({sql})
        }, 5000);
        if (!res.ok) continue;
        const txt = await res.text();
        let data; try { data = JSON.parse(txt); } catch { continue; }
        let rows = data.rows || [];
        if (data.results && data.results[0]?.response?.result) {
          const cols = data.results[0].response.result.cols.map(c=>c.name);
          rows = data.results[0].response.result.rows.map(r=>{ let o={}; r.forEach((c,i)=>{o[cols[i]]=c.value ?? c.text ?? "";}); return o; });
        }
        for (const r of rows) {
          const m = r["مفعلة"] ?? r["مفعله"] ?? r.enabled;
          if (m==0 || m=="0" || m===false || m=="false") {
            const result = {blocked:true, source:'turso-direct:'+sql.slice(0,50), row:r, api};
            cache.map.set(key, result);
            cache.time = Date.now();
            return result;
          } else if (m==1 || m=="1") {
            const result = {blocked:false, source:'turso-enabled', row:r};
            cache.map.set(key, result);
            cache.time = Date.now();
            return result;
          }
        }
      } catch(e) { continue; }
    }
  }

  const result = {blocked:false, source:'not-found', row:null};
  cache.map.set(key, result);
  cache.time = Date.now();
  return result;
}

const PHOTO = "data:image/jpeg;base64,";

function blockedHTML(pName, info) {
  const src = info.source || 'unknown';
  return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>مغلقة - `+pName+`</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@700;900&family=Cairo:wght@900&display=swap" rel="stylesheet"><style>
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:#0a0a1a;background:radial-gradient(ellipse at top,#1a1a40 0%,#0a0a1a 50%,#000 100%);font-family:Tajawal,Cairo,sans-serif;direction:rtl;display:flex;align-items:center;justify-content:center;overflow:hidden} 
.box{position:relative;z-index:2;display:flex;gap:32px;align-items:center;background:rgba(255,255,255,0.07);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.12);border-radius:24px;padding:32px;max-width:800px;width:92%}.phw{position:relative;width:180px;height:180px}.phi{width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid white}.ct{flex:1;text-align:right}.pn{font-size:14px;color:#a78bfa;background:rgba(167,139,250,0.15);padding:6px 12px;border-radius:8px;display:inline-block;margin-bottom:10px;font-family:monospace;direction:ltr}.src{font-size:10px;color:#9ca3af;background:rgba(0,0,0,0.3);padding:6px;border-radius:6px;margin-bottom:10px;word-break:break-all}
</style></head><body><div class="box"><div class="phw"><img src="`+PHOTO+`" class="phi"></div><div class="ct"><h2>الصفحة مغلقة</h2><div class="pn">`+pName+`</div><p>تم تعطيلها من لوحة التحكم (مفعلة=0)</p><div class="src">`+src+`</div><a href="/" style="color:white;background:#7850ff;padding:10px 20px;border-radius:10px;text-decoration:none;display:inline-block;margin-top:10px">🏠 الرئيسية</a></div></div></body></html>`;
}

export default {
  async fetch(req, env, ctx) {
    const url = new URL(req.url);
    const path = url.pathname;

    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        // جرب فحص أشهر الصفحات
        const testPages = ['tables.html','AddHafez1.html','salaryold.html','pageAdmin1.html','AddHafez2.html','index.html'];
        const blocked = [];
        const details = [];
        for (const p of testPages) {
          const info = await checkOnePage(p);
          if (info.blocked) blocked.push(p);
          details.push({page:p, blocked:info.blocked, source:info.source});
        }
        return new Response(JSON.stringify({blocked, details, time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
      if (path === '/api/debug') {
        const q = url.searchParams.get('page') || 'tables.html';
        const info = await checkOnePage(q);
        return new Response(JSON.stringify({query:q, info, cacheSize:cache.map.size, time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
      if (path === '/api/check-page-status') {
        try {
          const body = await req.json();
          const info = await checkOnePage(body.pageName || 'test');
          return new Response(JSON.stringify({blocked:info.blocked, source:info.source}), {
            headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
          });
        } catch(e) {
          return new Response(JSON.stringify({blocked:false, error:e.message}), {headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*'}});
        }
      }
    }

    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar'];
    if (skip.some(s=>path.toLowerCase().includes(s.toLowerCase()))) {
      return await env.ASSETS.fetch(req);
    }

    let pageName = path.split('/').pop() || 'index.html';
    if (path === '/' || path === '') pageName = 'index.html';
    const isHtml = path.endsWith('.html') || path==='/' || path==='' || (!path.includes('.') && !path.startsWith('/api/'));

    if (isHtml) {
      const info = await checkOnePage(pageName);
      if (info.blocked) {
        return new Response(blockedHTML(pageName, info), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'v16-check-api'}
        });
      }
    }

    return env.ASSETS.fetch(req);
  }
}
