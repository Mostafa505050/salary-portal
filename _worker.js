// _worker.js - يقرأ من turso-api مباشرة - ضع المتغيرات في turso-api فقط!
// salary-portal بدون متغيرات - يقرأ ديناميكي من turso-api
...
let cache = {pages:FALLBACK_BLOCKED, time:0, source:'fallback'};
const TTL = 10 * 1000; // 10 ثواني فقط ليكون ديناميكي سريع

async function getBlockedPages(env) {
  if (Date.now() - cache.time < TTL && cache.pages.length>=0) return cache;

  // 1- حاول قراءة مباشرة من Turso إذا كان عندك TURSO_URLL في salary-portal
  const urlNames = ['TURSO_URL', 'TURSO_URLL', 'TURSO_URLI', 'LIBSQL_URL'];
  const tokenNames = ['TURSO_TOKEN', 'TURSO_TOKENL', 'TURSO_TOKENI', 'LIBSQL_TOKEN'];
  let directUrl = "";
  let directToken = "";
  for (const n of urlNames) if (env[n]) { directUrl = env[n]; break; }
  for (const n of tokenNames) if (env[n]) { directToken = env[n]; break; }

  if (directUrl && directToken) {
    try {
      const httpUrl = directUrl.replace('libsql://', 'https://').replace('wss://','https://');
      ...
      if (res.ok) {
        ...
        if (data && data[0] && data[0].results) {
          ...
          if (idx>=0) {
            ...
            cache={pages, time:Date.now(), source:'turso-direct-salary-portal'};
            return cache;
          }
        }
      }
    } catch(e) { console.log('Direct Turso failed, trying turso-api'); }
  }

  // 2- إذا فشل المباشر، اقرأ من turso-api (اللي فيه المتغيرات)
  // turso-api يقرأ من جدول صفحات_الموقع ديناميكي
  const apiEndpoints = [
  ...
  for (const apiUrl of apiEndpoints) {
    try {
      const res = await fetch(apiUrl, {cf:{cacheTtl:5}});
      if (res.ok) {
        const d = await res.json();
        if (d.blocked) {
          const pages = d.blocked.map(s=>String(s).toLowerCase());
          cache={pages, time:Date.now(), source:'via-'+new URL(apiUrl).hostname, apiData:d};
          return cache;
        }
      }
    } catch(e) { console.log('API '+apiUrl+' failed'); }
  }

  // 3- احتياطي أخير
  cache={pages:FALLBACK_BLOCKED, time:Date.now(), source:'fallback-hardcoded'};
  return cache;
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        const result = await getBlockedPages(env);
        ...
        return new Response(JSON.stringify({
          ...
          column:'مفعلة', 
          rule:'0=منع,1=سماح', 
          hasEnv: hasLocal || result.source.includes('turso-api') || result.source.includes('via-'),
          ...
          note: result.source.includes('turso-api') ? '✅ يقرأ ديناميكي من turso-api (المتغيرات في turso-api)' : (hasLocal ? '✅ مباشر من salary-portal' : '⚠️ احتياطي - أضف المتغيرات في turso-api'),
    ...
    if (isHtml) {
      ...
      if (isBlocked(pageName, result.pages)) {
        const info = {
          ...
          label: result.source.includes('turso-api') ? '✅ عبر turso-api (ديناميكي)' : (result.source.includes('direct') ? '✅ مباشر' : '⚠️ احتياطي'),
          source: result.source
        };
