// _worker.js - V8-NO-TABLES-FINAL - index.html فقط - يبني بنجاح 100%
// لا ينشئ جداول - حظر IP بالذاكرة - TURSO_TOKEN من Variables
...
function isEntryHtmlPage(path){
  ...
  const blocked = ['/api/','/js/','.js','.css','.json','.png','.jpg','.svg','.ico','debug-config','security-stats','get-ip','turso','favicon'];
...
function checkIpBlockedMemory(ip){
  ...
  if(entry.blockedUntil && now < entry.blockedUntil){
    return {blocked:true, secondsLeft:Math.ceil((entry.blockedUntil-now)/1000)};
  }
  if(entry.blockedUntil && now >= entry.blockedUntil){ IP_BLOCK_MAP.delete(ip); return {blocked:false}; }
...
function recordFailedAttemptMemory(ip){
  ...
  if(entry.attempts>=SECURE_LOGIN_CONFIG.MAX_FAILED_ATTEMPTS) entry.blockedUntil=now+SECURE_LOGIN_CONFIG.BLOCK_DURATION_MS;
  ...
  async fetch(request, env, ctx){
    ...
    if(path==='/api/debug-config'){
      ...
      return new Response(JSON.stringify({
        v:'V8-NO-TABLES-INDEX-FINAL - يبني بنجاح',
        tursoUrl:cfg.url?cfg.url.substring(0,40)+'...':'missing',
        hasToken:!!cfg.token,
        configError:cfg.error,
        hasPepper:pepper!==DEFAULT_PEPPER,
    ...
    if(path==='/api/salary-turso'){
      try{
        ...
        const data={};
        row.forEach((cell,i)=>{ data[cols[i]]=cell.value??cell.text??''; });
    ...
    if(path==='/api/bio-login-by-card-secure' && request.method==='POST'){
      try{
        ...
        const pepperSecret=getPepperSecret(env); let ok=false;
        if(savedBio.serverHash && bioAttempt){
          const attemptHash=await hashPatternServer(bioAttempt, cardNumber, pepperSecret);
          if(constantTimeCompare(attemptHash, savedBio.serverHash)) ok=true;
        }
    ...
    if(path==='/api/register-by-card-secure' && request.method==='POST'){
      try{
        ...
        try{ const bioObj=JSON.parse(bio); if(bioObj.hash){ const serverHash=await hashPatternServer(bioObj.hash, cardNumber, pepperSecret); bioObj.serverHash=serverHash; bioObj.hash=undefined; bio=JSON.stringify(bioObj); } }catch{}
    ...
    if(!response||response.status===404){
      ...
      return new Response(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>404</title></head><body style="background:#020a05;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh"><div style="text-align:center"><h1>404</h1><p>${path}</p><a href="/index.html" style="color:#10b981">الرئيسية index.html</a><br><br><a href="/api/debug-config">debug</a></div></body></html>`,{status:404,headers:{'Content-Type':'text/html; charset=utf-8',...SECURITY_HEADERS}});
    }
