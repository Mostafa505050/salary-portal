// _worker.js FINAL - متوافق 100% مع wrangler.toml الحالي
// wrangler.toml: main="_worker.js", [assets] directory="./", binding="ASSETS", run_worker_first=true
// يقرأ من جدول صفحات_الموقع مفعلة=0 منع / 1 سماح + تصميم خيالي + صورتك + بدون زر تحكم
...
// احتياطي من Database-Manager - يعمل حتى بدون env
...
async function getBlockedFromTurso(env) {
  ...
  } catch(e) { console.log('Turso direct failed:', e.message); }
  return null;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        ...
        return new Response(JSON.stringify({
          ...
          rule:'0=منع,1=سماح', 
          hasEnv,
          hasUrl:!!(env.TURSO_URL||env.LIBSQL_URL),
          hasToken:!!(env.TURSO_TOKEN||env.LIBSQL_TOKEN),
          source: hasEnv ? 'turso-direct' : 'fallback',
          time:new Date().toISOString(),
          note: hasEnv ? '✅ ديناميكي من Turso' : '⚠️ احتياطي - أضف TURSO_URL و TURSO_TOKEN في Workers Settings'
        }, null, 2), {
      ...
      if (path === '/api/debug') {
        ...
        return new Response(JSON.stringify({
          ...
          blocked,
          blockedList:pages,
          hasEnv:!!(env.TURSO_URL||env.LIBSQL_URL),
          table:'صفحات_الموقع',
          time:new Date().toISOString()
        }, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
    ...
    // لا تحجب ملفات النظام
    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar', 'protect.js', 'wrangler.toml', '.assetsignore'];
    ...
    if (isHtml) {
      ...
      const hasEnv = String(!!(env.TURSO_URL || env.LIBSQL_URL));
      if (isBlocked(pageName, blockedList)) {
        return new Response(fantasyBlockedPage(pageName, blockedList, hasEnv), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache, no-store','X-Blocked-By':'worker-final-wrangler-compatible'}
        });
      }
    }
