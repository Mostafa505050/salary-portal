// _worker.js FINAL v21 - يعمل بكفاءة مع Database-Manager.html و جدول صفحات_الموقع
// مفعلة=0 منع، مفعلة=1 سماح - تصميم خيالي + صورتك + بدون زر تحكم
// يتناسب 100% مع Database-Manager.html المرفق

// const PHOTO = "data:image/jpeg;base64,";

// Cache
let cache = {pages:[], time:0};
const TTL = 15 * 1000; // 15 ثانية كما في Database-Manager

async function getBlockedFromTurso(env) {
  // يقرأ من نفس Turso الذي يكتب فيه Database-Manager
  const url = env.TURSO_URL || env.LIBSQL_URL || "";
  const token = env.TURSO_TOKEN || env.LIBSQL_TOKEN || env.TURSO_AUTH_TOKEN || "";
  
  if (url && token) {
    try {
      const httpUrl = url.replace('libsql://', 'https://').replace('wss://','https://');
      const res = await fetch(httpUrl, {
        method:'POST',
        headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},
        body: JSON.stringify({statements:["SELECT \"اسم_الصفحة\" FROM \"صفحات_الموقع\" WHERE \"مفعلة\"=0"]})
      });
      if (res.ok) {
        const data = await res.json();
        if (data && data[0] && data[0].results) {
          const cols = data[0].results.columns || [];
          const rows = data[0].results.rows || [];
          const pages = rows.map(r=>{ let idx=cols.indexOf("اسم_الصفحة"); return idx>=0?String(r[idx]||"").toLowerCase().trim(): ""; }).filter(Boolean);
          if (pages.length>=0) return pages;
        }
      }
    } catch(e) {}
  }

  // Fallback: عبر auth-api (إذا كان Database-Manager يكتب هناك)
  try {
    const res = await fetch("https://auth-api.mostafa-voic77729.workers.dev/api/blocked-list");
    if (res.ok) {
      const d = await res.json();
      if (d.blocked && Array.isArray(d.blocked)) return d.blocked.map(s=>s.toLowerCase());
    }
  } catch(e) {}

  return [];
}

async function getBlockedPages(env) {
  if (Date.now() - cache.time < TTL && cache.pages.length>0) return cache.pages;
  
  // 1. جرب Turso مباشر
  try {
    const pages = await getBlockedFromTurso(env);
    if (pages.length>0 || (Date.now() - cache.time > TTL)) {
      cache={pages, time:Date.now()};
      if (pages.length>0) return pages;
    }
  } catch(e) {}

  // 2. Fallback API
  try {
    const res = await fetch("https://auth-api.mostafa-voic77729.workers.dev/api/turso", {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({sql:`SELECT "اسم_الصفحة" FROM "صفحات_الموقع" WHERE "مفعلة"=0`})
    });
    if (res.ok) {
      const txt = await res.text();
      let data; try { data=JSON.parse(txt); } catch {}
      if (data) {
        let rows = data.rows || [];
        if (data.results && data.results[0]?.response?.result) {
          const cols = data.results[0].response.result.cols.map(c=>c.name);
          rows = data.results[0].response.result.rows.map(r=>{ let o={}; r.forEach((c,i)=>{o[cols[i]]=c.value??c.text??"";}); return o; });
        }
        const pages = rows.map(r=>String(r["اسم_الصفحة"]||"").toLowerCase().trim()).filter(Boolean);
        cache={pages, time:Date.now()};
        return pages;
      }
    }
  } catch(e) {}

  return cache.pages;
}

async function isBlocked(env, pageName) {
  const low = pageName.toLowerCase().replace('.html','').trim();
  const pages = await getBlockedPages(env);
  for (const b of pages) {
    const bl = b.toLowerCase().replace('.html','').trim();
    if (bl===low || bl===low+'.html' || low.includes(bl) || bl.includes(low)) {
      return {blocked:true, source:'turso:'+b, list:pages};
    }
  }
  return {blocked:false, source:'allowed', list:pages};
}

function fantasyBlockedPage(pageName, info) {
  return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>الصفحة مغلقة - `+pageName+`</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@700;900&family=Cairo:wght@900&display=swap" rel="stylesheet"><style>
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:#0a0a1a;background:radial-gradient(ellipse at top,#1a1a40 0%,#0a0a1a 50%,#000 100%);font-family:Tajawal,Cairo,sans-serif;direction:rtl;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
body::before{content:'';position:absolute;inset:0;background:radial-gradient(circle at 20% 30%,rgba(120,80,255,0.18) 0%,transparent 50%),radial-gradient(circle at 80% 70%,rgba(255,80,180,0.14) 0%,transparent 50%)}
.stars{position:absolute;inset:0;background-image:radial-gradient(2px 2px at 20px 30px,#fff,transparent),radial-gradient(2px 2px at 40px 70px,rgba(255,255,255,0.8),transparent);background-repeat:repeat;background-size:200px 200px;opacity:0.25;animation:s 120s linear infinite}@keyframes s{from{transform:translateY(0)}to{transform:translateY(-200px)}}
.box{position:relative;z-index:2;display:flex;gap:48px;align-items:center;background:rgba(255,255,255,0.07);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.12);border-radius:32px;padding:48px;max-width:920px;width:92%;box-shadow:0 20px 60px rgba(0,0,0,0.5),0 0 80px rgba(120,80,255,0.18);animation:f 6s ease-in-out infinite}@keyframes f{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}
.ph{position:relative;flex-shrink:0}.phw{position:relative;width:220px;height:220px}.phg{position:absolute;inset:-10px;border-radius:50%;background:conic-gradient(from 0deg,#7850ff,#ff50b4,#50c8ff,#7850ff);animation:r 4s linear infinite;filter:blur(14px);opacity:0.85}@keyframes r{to{transform:rotate(360deg)}}.phb{position:absolute;inset:-4px;border-radius:50%;background:conic-gradient(from 0deg,#7850ff,#ff50b4,#50c8ff,#7850ff);animation:r 4s linear infinite reverse}.phi{position:relative;width:100%;height:100%;border-radius:50%;object-fit:cover;border:4px solid rgba(255,255,255,0.95);box-shadow:0 8px 32px rgba(0,0,0,0.5);z-index:2}.phd{position:absolute;bottom:6px;right:6px;width:52px;height:52px;background:linear-gradient(135deg,#ef4444,#dc2626);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:26px;border:3px solid white;z-index:3;box-shadow:0 4px 16px rgba(239,68,68,0.5);animation:bp 2s ease-in-out infinite}@keyframes bp{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
.ct{flex:1;text-align:right}.ct h1{font-family:Cairo,sans-serif;font-size:32px;font-weight:900;color:white;margin-bottom:10px;background:linear-gradient(135deg,#fff,#c4b5fd);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.pn{font-size:16px;font-weight:700;color:#a78bfa;margin-bottom:12px;font-family:monospace;direction:ltr;text-align:right;background:rgba(167,139,250,0.15);padding:8px 14px;border-radius:10px;border:1px solid rgba(167,139,250,0.3);display:inline-block}.msg{font-size:14px;color:#d1d5db;line-height:1.8;margin-bottom:14px}.src{font-size:11px;color:#9ca3af;background:rgba(0,0,0,0.3);padding:8px 12px;border-radius:8px;margin-bottom:16px;direction:ltr;font-family:monospace}.fts{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px}.ft{background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);padding:6px 10px;border-radius:8px;font-size:11px;color:#9ca3af}.hb{display:inline-flex;align-items:center;gap:8px;padding:12px 24px;background:linear-gradient(135deg,#7850ff,#5040ff);color:white;text-decoration:none;border-radius:12px;font-weight:800;font-size:13px;box-shadow:0 8px 24px rgba(120,80,255,0.35)}@media (max-width:768px){.box{flex-direction:column;padding:28px 20px;gap:24px;text-align:center}.ct{text-align:center}.phw{width:180px;height:180px;margin:0 auto}}
</style></head><body><div class="stars"></div><div class="box"><div class="ph"><div class="phw"><div class="phg"></div><div class="phb"></div><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"><div class="phd">🔒</div></div></div><div class="ct"><h1>الصفحة مغلقة</h1><div class="pn">`+pageName+`</div><p class="msg">عذراً، هذه الصفحة مغلقة حالياً من قبل الإدارة.<br>تم تعطيلها من لوحة التحكم (مفعلة=0) في جدول صفحات_الموقع.</p><div class="src">Source: `+info.source+`<br>Blocked: `+info.list.join(', ')+`</div><div class="fts"><div class="ft">🛡️ حماية من السيرفر</div><div class="ft">✨ تصميم خيالي</div><div class="ft">🔒 Turso: مفعلة=0</div></div><a href="/" class="hb">🏠 العودة للرئيسية</a></div></div></body></html>`;
}

export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    const path = url.pathname;

    // APIs تشخيص
    if (path.startsWith('/api/')) {
      if (path === '/api/blocked-list') {
        const pages = await getBlockedPages(env);
        return new Response(JSON.stringify({blocked:pages, count:pages.length, table:'صفحات_الموقع', column:'مفعلة', rule:'0=منع,1=سماح', hasEnv:!!(env.TURSO_URL||env.LIBSQL_URL), time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
      if (path === '/api/debug') {
        const q = url.searchParams.get('page') || 'AddHafez1.html';
        const info = await isBlocked(env, q);
        return new Response(JSON.stringify({query:q, isBlocked:info, table:'صفحات_الموقع', time:new Date().toISOString()}, null, 2), {
          headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'no-cache'}
        });
      }
    }

    // صفحات لا تُحجب أبداً
    const skip = ['database-manager', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar', 'protect.js'];
    if (skip.some(s=>path.toLowerCase().includes(s.toLowerCase()))) {
      return await env.ASSETS.fetch(request);
    }

    let pageName = path.split('/').pop() || 'index.html';
    if (path === '/' || path === '') pageName = 'index.html';
    const isHtml = path.endsWith('.html') || path==='/' || path==='' || (!path.includes('.') && !path.startsWith('/api/'));

    if (isHtml) {
      const info = await isBlocked(env, pageName);
      if (info.blocked) {
        return new Response(fantasyBlockedPage(pageName, info), {
          status:403,
          headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache, no-store','X-Blocked-By':'v21-Turso-صفحات_الموقع'}
        });
      }
    }

    return env.ASSETS.fetch(request);
  }
}
