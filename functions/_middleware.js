// functions/_middleware.js - هذا يعمل دائماً في Cloudflare Pages
// يمنع الصفحات التي مفعلة=0 في جدول صفحات_الموقع - تصميم خيالي + صورتك + بدون تحكم
...
const FALLBACK_BLOCKED = ['addhafez1.html', 'addhafez1', 'tables.html', 'tables', 'salaryold.html'];

let cache = {pages:[], time:0};
const TTL = 15 * 1000;

async function getBlocked(context) {
  if (Date.now() - cache.time < TTL && cache.pages.length>0) return cache.pages;

  // جرب قراءة من Turso عبر env
...
function blockedHTML(pageName, blockedList) {
...
.box{display:flex;gap:32px;align-items:center;background:rgba(255,255,255,0.07);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.12);border-radius:24px;padding:32px;max-width:800px;width:92%}.phw{width:180px;height:180px;position:relative}.phi{width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid white}.pn{font-size:14px;color:#a78bfa;background:rgba(167,139,250,0.15);padding:6px 12px;border-radius:8px;display:inline-block;margin-bottom:10px;font-family:monospace;direction:ltr}.src{font-size:10px;color:#9ca3af;background:rgba(0,0,0,0.3);padding:6px;border-radius:6px;margin-bottom:10px}
</style></head><body><div class="box"><div class="phw"><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"></div><div><h2>الصفحة مغلقة</h2><div class="pn">`+pageName+`</div><p>تم تعطيلها من لوحة التحكم (مفعلة=0)</p><div class="src">`+blockedList.join(', ')+`</div><a href="/" style="color:white;background:#7850ff;padding:10px 20px;border-radius:10px;text-decoration:none;display:inline-block;margin-top:10px">🏠 الرئيسية</a></div></div></body></html>`;
}
...
export async function onRequest(context) {
  ...
  if (isHtml) {
    const blockedList = await getBlocked(context);
    if (isBlocked(pageName, blockedList)) {
      return new Response(blockedHTML(pageName, blockedList), {
        status:403,
        headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'functions-middleware'}
      });
    }
  }
