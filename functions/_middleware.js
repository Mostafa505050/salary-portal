// functions/_middleware.js - FINAL FIX - يعمل حتى بدون TURSO_URL
// hasEnv=false -> يستخدم قائمة احتياطية من Database-Manager

const PHOTO = "data:image/jpeg;base64,";

// من صورتك image_c3958f.png: 2 صفحات معطلة
const FALLBACK_BLOCKED = ['addhafez1.html', 'addhafez1', 'tables.html', 'tables', 'salaryold.html', 'salaryold', 'addhafez1.html'];

let cache = {pages:FALLBACK_BLOCKED, time:Date.now()};
const TTL = 15 * 1000;

async function getBlockedList(context) {
  // إذا كان Cache موجود وحديث، استخدمه
  if (Date.now() - cache.time < TTL && cache.pages.length>0) return cache.pages;

  const env = context.env || {};
  const url = env.TURSO_URL || env.LIBSQL_URL || "";
  const token = env.TURSO_TOKEN || env.LIBSQL_TOKEN || env.TURSO_AUTH_TOKEN || "";

  // 1. Turso مباشر (إذا hasEnv=true)
  if (url && token) {
    try {
      const httpUrl = url.replace('libsql://', 'https://').replace('wss://','https://');
      const res = await fetch(httpUrl, {
        method:'POST',
        headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json'},
        body: JSON.stringify({statements:["SELECT \"اسم_الصفحة\" FROM \"صفحات_الموقع\" WHERE \"مفعلة\"=0"]})
      });
      if (res.ok) {
        const data = await res.json();
        if (data && data[0] && data[0].results) {
          const cols = data[0].results.columns || [];
          const idx = cols.indexOf("اسم_الصفحة");
          if (idx>=0) {
            const pages = data[0].results.rows.map(r=>String(r[idx]||"").toLowerCase().trim()).filter(Boolean);
            if (pages.length>0) {
              cache={pages, time:Date.now()};
              return pages;
            }
          }
        }
      }
    } catch(e) {}
  }

  // 2. auth-api
  try {
    const res = await fetch("https://auth-api.mostafa-voic77729.workers.dev/api/blocked-list");
    if (res.ok) {
      const d = await res.json();
      if (d.blocked && d.blocked.length>0) {
        cache={pages:d.blocked.map(s=>s.toLowerCase()), time:Date.now()};
        return cache.pages;
      }
    }
  } catch(e) {}

  // 3. احتياطي دائم من صورتك - يعمل حتى بدون env
  cache={pages:FALLBACK_BLOCKED, time:Date.now()};
  return FALLBACK_BLOCKED;
}

function isBlocked(pageName, list) {
  const low = pageName.toLowerCase().replace('.html','').trim();
  return list.some(b=>{
    const bl = b.toLowerCase().replace('.html','').trim();
    return bl===low || low.includes(bl) || bl.includes(low);
  });
}

function blockedPage(pageName, list, hasEnv) {
  return `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>مغلقة - `+pageName+`</title><style>
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:#0a0a1a;background:radial-gradient(ellipse at top,#1a1a40 0%,#0a0a1a 50%,#000 100%);font-family:Tajawal,sans-serif;direction:rtl;display:flex;align-items:center;justify-content:center;overflow:hidden}
.box{position:relative;z-index:2;display:flex;gap:32px;align-items:center;background:rgba(255,255,255,0.07);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.12);border-radius:24px;padding:32px;max-width:800px;width:92%;box-shadow:0 20px 60px rgba(0,0,0,0.5)}.phw{width:180px;height:180px;position:relative}.phi{width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid white}.pn{font-size:14px;color:#a78bfa;background:rgba(167,139,250,0.15);padding:6px 12px;border-radius:8px;display:inline-block;margin-bottom:10px;font-family:monospace;direction:ltr}.env{font-size:10px;padding:4px 8px;border-radius:6px;margin-bottom:8px;display:inline-block}.env.true{background:#ecfdf5;color:#065f46}.env.false{background:#fef2f2;color:#991b1b}
</style></head><body><div class="box"><div class="phw"><img src="`+PHOTO+`" class="phi" alt="مصطفي درويش"></div><div><h2>🔒 الصفحة مغلقة</h2><div class="pn">`+pageName+`</div><div class="env `+hasEnv+`">hasEnv: `+hasEnv+` - `+(hasEnv==='true'?'✅ ديناميكي من Turso':'⚠️ احتياطي - أضف TURSO_URL')+`</div><p>تم تعطيلها من لوحة التحكم (مفعلة=0) في جدول صفحات_الموقع</p><div style="font-size:11px;background:rgba(0,0,0,0.3);padding:8px;border-radius:6px;margin:10px 0;direction:ltr">Blocked: `+list.join(', ')+`</div><a href="/" style="color:white;background:#7850ff;padding:10px 20px;border-radius:10px;text-decoration:none;display:inline-block;margin-top:10px">🏠 الرئيسية</a></div></div></body></html>`;
}

export async function onRequest(context) {
  const url = new URL(context.request.url);
  const path = url.pathname;

  const skip = ['database-manager', 'api', 'turso-api', 'hafez-api', 'auth-api', 'auth', 'favicon', '.js', '.css', '.json', '.png', '.jpg', '.svg', '.ico', '.woff', 'fonts.googleapis', 'sidebar', 'protect.js'];
  if (skip.some(s=>path.toLowerCase().includes(s.toLowerCase()))) {
    return await context.next();
  }

  let pageName = path.split('/').pop() || 'index.html';
  if (path === '/' || path === '') pageName = 'index.html';
  const isHtml = path.endsWith('.html') || path==='/' || path==='' || (!path.includes('.') && !path.startsWith('/api/'));

  if (isHtml) {
    const list = await getBlockedList(context);
    const hasEnv = !!(context.env && (context.env.TURSO_URL || context.env.LIBSQL_URL));
    if (isBlocked(pageName, list)) {
      return new Response(blockedPage(pageName, list, String(hasEnv)), {
        status:403,
        headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-cache','X-Blocked-By':'middleware-FINAL-FALLBACK'}
      });
    }
  }

  return await context.next();
}
